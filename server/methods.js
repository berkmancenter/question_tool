...
Meteor.methods({
    ...
    Questions.insert(
      {
        ...
        tablename,
        text:
          "Welcome to the Q&A tool. Please post on this instance. Vote by clicking on the upvote icon to raise a post's prominence. Reply or share a post on facebook and twitter by clicking on the respective icons.",
        poster: "the system",
        timeorder: new Date().getTime() - 1000,
        lasttouch: new Date().getTime() - 1000,
        state: "normal",
  ...
  unhide(instanceid) {
    if (this.userId) {
      let keys;
      const email = Meteor.users.findOne({ _id: this.userId }).emails[0]
        .address;
      const instance = Instances.findOne({
        _id: instanceid
      });
      if (
        email === instance.admin ||
        instance.moderators.indexOf(email) !== -1
      ) {
  ...
  unhideThis(id) {
    if (this.userId) {
      ...
      if (
        email !== table.admin &&
        table.moderators &&
        table.moderators.indexOf(email) === -1
      ) {
        return false;
      }
      Questions.update(
  ...
  canModify(question) {
    if (this.userId) {
      ...
      return (
        (quest.email === email && quest.posterLoggedIn) ||
        email === inst.admin ||
        inst.moderators.indexOf(email) !== -1
      );
    }
    return false;
  },
  ...
  modify(question, id) {
    ...
    if (this.userId) {
      ...
      if (!email || !instance || !instance.admin || !quest) {
        return false;
      }
      if (
        email !== instance.admin &&
  ...
  combine(question, id1, id2) {
    if (this.userId) {
      ...
      if (q1.instanceid !== q2.instanceid) {
        return false;
      }
  ...
  adminRemove(instanceid) {
    ...
    if (this.userId) {
      ...
      if (
        email &&
        (email === table.admin || email === process.env.SUPERADMIN_EMAIL)
      ) {
  ...
  vote(questionid) {
    ...
    if (this.userId) {
      votes = Votes.findOne({
        $or: [{ qid: questionid, ip }, { qid: questionid, uid: this.userId }]
      });
    } else {
  ...
  hideThis(id) {
    if (this.userId) {
      ...
      if (
        email === table.admin ||
        (table.moderators && table.moderators.indexOf(email) !== -1)
      ) {
        Questions.update(
          { _id: id },
          { $set: { state: "disabled" } },
  ...
  addFavorite(id) {
    if (this.userId) {
      let success = true;
      const faves = Meteor.users.findOne({ _id: this.userId }).profile
        .favorites;
      if (!Instances.findOne({ _id: id })) {
        return false;
      }
      if (faves && faves.indexOf(id) !== -1) {
        return false;
      }
      Meteor.users.update(
        { _id: this.userId },
        { $push: { "profile.favorites": id } },
        (error, count, status) => {
          if (error) {
            success = false;
          }
