  ...
  'click #buttonarea': function (event, template) {
      ...
      if (!posterEmail) {
        ...
        Meteor.call('register', posterEmail, password2, posterName, (error, result) => {
          if (result === 3) {
            ...
            Meteor.loginWithPassword(posterEmail, password2, (error) => {
              if (error) {
    ...
    Meteor.call('getIP', (error, result) => {
      if (!error) {
        ...
        Meteor.call('propose', Session.get('id'), Session.get('tablename'), question, posterName, posterEmail, result, (error, result) => {
