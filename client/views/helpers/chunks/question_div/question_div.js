...
Template.question_div.helpers({
  isPoster() {
    return (
      Meteor.user() &&
      Meteor.user().emails[0].address === this.email &&
      this.posterLoggedIn
    );
  },
  ...
  voteCount() {
    return Questions.findOne({ _id: this._id }).votes; // Query to fetch the number of upvotes done on any Question for an Instance
  },
  ...
  "keyup .replyarea": function(event, template) {
    ...
    if (found) {
      ...
      $(event.target).attr(
        "maxlength",
        Number(
          Instances.findOne({ _id: template.data.instanceid }).max_response +
            totalURL -
            found.length
        )
      );
