        ...
        .draggable({
          ...
          onend(event) {
            ...
            event.target.style.cssText =
              "-webkit-transform: translate(0px, 0px);z-index:0!important;";
...
function showListError(reason) {
  ...
  currentError = Blaze.renderWithData(
    Template.form_error,
    reason,
    parentNode,
    nextNode
...
function showReplyError(reason, id) {
  ...
  replyError = Blaze.renderWithData(
    Template.form_error,
    reason,
    parentNode,
    nextNode
...
Template.list.onCreated(function() {
  ...
  const adminMod =
    Meteor.user() &&
    (Template.instance().data.admin === Meteor.user().emails[0].address ||
      Template.instance().data.moderators.indexOf(
  ...
  this.visibleAnswers = new Mongo.Collection("visibleAnswers", {
    connection: null
  ...
  this.updateInPlace = questions => {
    questions.forEach(question =>
      this.visibleQuestions.update(
        { _id: question._id },
        { $set: { state: question.state } }
      )
    );
  };
  ...
  this.autorun(computation => {
    ...
    const hiddenQs = Questions.find({
      instanceid: template.data._id,
      state: "disabled"
    }).count();
    const answers = Answers.find({ instanceid: template.data._id }).fetch();
    const client = Template.instance()
      .visibleQuestions.find({ instanceid: template.data._id })
      .fetch();
    const clientHidden = Template.instance()
    ...
    if (
      ...
      this.state.get("presentMode") !== true &&
      updatedQs
    ) {
      this.state.set("hasChanges", true);
      if (!this.countdown) {
        this.countdown = Meteor.setTimeout(() => {
          this.onShowChanges(true);
        }, 5000);
        this.seconds.set(5);
        this.secondsInterval = Meteor.setInterval(() => {
...
Template.list.onRendered(() => {
  ...
  $("head").append(
    '<link rel="alternate" type="application/rss+xml" href="/rss/{{tablename}}"/>'
  );
});
...
Template.list.helpers({
  ...
  admin() {
    return (
      Meteor.user() &&
      Meteor.user().emails[0].address === Template.instance().data.admin
    );
  },
  moderator() {
    return (
  ...
  hasSeconds() {
    return (
      Template.instance().seconds.get() > 0 &&
      !Template.instance().state.get("typing")
    );
  },
  seconds() {
    ...
    for (let i = 0; i < questions.length; i++) {
      if (questions[i].state !== "disabled" || tableMod || tableAdmin) {
        ...
        questions[i].text = questions[i].text.replace(urlRegex, url => {
          ...
          if (!hasPeren) {
            return (
              '<a target="_blank" class="questionLink" rel="nofollow" href="' +
              fullURL +
              '">' +
              url +
              "</a>"
        ...
        questions[i].shade =
          "vc" + Math.round(3 + (questions[i].votes - avg) / stddev);
        // Sets the age marker depending on how long since question last modified
        const staleDiff =
          (Session.get("timeval") - questions[i].lasttouch) / 1000;
        const newDiff =
          (Session.get("timeval") - questions[i].timeorder) / 1000;
        ...
        const answers = Template.instance()
          .visibleAnswers.find({
            qid: questions[i]._id
          })
          .fetch();
        if (answers.length > 0) {
          ...
          for (let a = 0; a < questions[i].answer.length; a++) {
            ...
            // }
            questions[i].answer[a].text = questions[i].answer[a].text.replace(
              /\B(@\S+)/g,
              "<strong>$1</strong>"
            );
            questions[i].answer[a].text = questions[i].answer[a].text.replace(
...
Template.list.events({
  ...
  "click .voteright": function(event, template) {
    Meteor.call("vote", event.currentTarget.id, (error, result) => {
      ...
      if (typeof result === "object") {
        ...
        const errorCodes = {
          votedbefore:
            "It appears that you have already voted up this question.",
          lasttouch:
            "There was an error retrieving the time. Please return to the list and try again.",
          votes:
            "There was an error incrementing the votes. Please return to the list and try again.",
          qid:
            "There was an error with the question ID. Please return to the list and try again.",
          ip:
            "There was an error with your IP address. Please return to the list and try again.",
          tablename:
            "There was an error with the table name. Please return to the list and try again."
        };
  ...
  "click .adminquestionhide": function(event, template) {
    ...
    if (
      Questions.findOne({ _id: event.currentTarget.id }).state === "disabled"
    ) {
  ...
  "click #deletebutton": function(event, template) {
    popoverTemplate = Blaze.renderWithData(
      Template.delete,
      Instances.findOne({ _id: $(event.target.parentElement).data("tableId") }),
      document.getElementById("nav")
    );
  },
  "click #navAsk": function(event, template) {
    const parentNode = document.getElementById("nav-wrapper");
    dropDownTemplate = Blaze.renderWithData(
      Template.propose,
      template.data,
      parentNode
    );
  ...
  "click .facebookbutton": function(event, template) {
    popupwindow(
      "https://www.facebook.com/sharer/sharer.php?u=" +
        encodeURIComponent(
          window.location.origin + "/list/" + template.data.slug
        ),
      "Share Question Tool!",
      600,
      400
  ...
  "click .twitterbutton": function(event, template) {
    ...
    const tweetText =
      ...
      questionText +
      '..." on Question Tool by @berkmancenter ' +
